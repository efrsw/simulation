...
export class Graph {
    ...
    #randomConnection = 0.95
    #socialProbability = 0.3
    #focalProbability = 0.2
    ...
    #fillSimulationsArray() {
        for (let i = 1; i <= this.#numberOfSimulations; i++) {
            ...
            for (let [key, value] of newState) {
                ...
                for (let k = 0; k < this.#numberOfPeople; k++) {
                    ...
                    if (newState.get(`P${k}${value.source.id}`)?.state === 'present'
                        && newState.get(`P${k}${value.target.id}`)?.state === 'present'
                        && this.#socialProbability < Math.random()) {
                ...
                for (let k = 0; k < this.#numberOfPeople; k++) {
                    ...
                    if (newState.get(`F${k}${value.source.id}`)?.state === 'present'
                        && newState.get(`F${k}${value.target.id}`)?.state === 'present'
                        && this.#focalProbability < Math.random()) {
        ...
        this.simulations.push(this.#solidifyLinks(this.simulations[this.simulations.length - 1]));
    }
        ...
        for (let i of this.links.keys()) {
            ...
            switch (link.target.type) {
                case 'foci':
                    this.links.set(i, {
                        ...link,
                        state: this.#focalProbability > Math.random() ? 'present' : 'absent'
                    });
                    break;
                case 'person':
                    this.links.set(i, {
                        ...link,
                        state: this.#socialProbability > Math.random() ? 'present' : 'absent'
                    });
            }
        }
    }
